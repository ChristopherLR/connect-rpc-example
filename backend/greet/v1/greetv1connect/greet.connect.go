...
import (
	...
	context "context"
	errors "errors"
	v1 "example.com/connect-example/greet/v1"
	http "net/http"
	strings "strings"
)
...
const (
	...
	// GreetServiceStreamGreetingsProcedure is the fully-qualified name of the GreetService's
	// StreamGreetings RPC.
	GreetServiceStreamGreetingsProcedure = "/greet.v1.GreetService/StreamGreetings"
)
...
type GreetServiceClient interface {
	...
	StreamGreetings(context.Context, *connect.Request[v1.GreetRequest]) (*connect.ServerStreamForClient[v1.GreetResponse], error)
}
...
func NewGreetServiceClient(httpClient connect.HTTPClient, baseURL string, opts ...connect.ClientOption) GreetServiceClient {
	...
	return &greetServiceClient{
		...
		streamGreetings: connect.NewClient[v1.GreetRequest, v1.GreetResponse](
			httpClient,
			baseURL+GreetServiceStreamGreetingsProcedure,
			connect.WithSchema(greetServiceMethods.ByName("StreamGreetings")),
			connect.WithClientOptions(opts...),
		),
	}
}
...
type greetServiceClient struct {
	...
	streamGreetings *connect.Client[v1.GreetRequest, v1.GreetResponse]
}
...
// StreamGreetings calls greet.v1.GreetService.StreamGreetings.
func (c *greetServiceClient) StreamGreetings(ctx context.Context, req *connect.Request[v1.GreetRequest]) (*connect.ServerStreamForClient[v1.GreetResponse], error) {
	return c.streamGreetings.CallServerStream(ctx, req)
}

// GreetServiceHandler is an implementation of the greet.v1.GreetService service.
type GreetServiceHandler interface {
	Greet(context.Context, *connect.Request[v1.GreetRequest]) (*connect.Response[v1.GreetResponse], error)
	StreamGreetings(context.Context, *connect.Request[v1.GreetRequest], *connect.ServerStream[v1.GreetResponse]) error
}
...
func NewGreetServiceHandler(svc GreetServiceHandler, opts ...connect.HandlerOption) (string, http.Handler) {
	...
	greetServiceStreamGreetingsHandler := connect.NewServerStreamHandler(
		GreetServiceStreamGreetingsProcedure,
		svc.StreamGreetings,
		connect.WithSchema(greetServiceMethods.ByName("StreamGreetings")),
		connect.WithHandlerOptions(opts...),
	)
	return "/greet.v1.GreetService/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case GreetServiceStreamGreetingsProcedure:
			greetServiceStreamGreetingsHandler.ServeHTTP(w, r)
		default:
			http.NotFound(w, r)
		}
	})
...
func (UnimplementedGreetServiceHandler) StreamGreetings(context.Context, *connect.Request[v1.GreetRequest], *connect.ServerStream[v1.GreetResponse]) error {
	return connect.NewError(connect.CodeUnimplemented, errors.New("greet.v1.GreetService.StreamGreetings is not implemented"))
}
